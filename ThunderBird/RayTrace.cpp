...
void render(const std::vector<Sphere> &spheres)
{
	...
	// KSH - Save to BMP File //
	unsigned char* imgbuff = new unsigned char[width*height * 3];
	for (unsigned i = 0; i < width * height; ++i) {
		imgbuff[i * 3] = (unsigned char)(std::min(float(1), image[i].x) * 255);
		imgbuff[i * 3 + 1] = (unsigned char)(std::min(float(1), image[i].y) * 255);
		imgbuff[i * 3 + 2] = (unsigned char)(std::min(float(1), image[i].z) * 255);
...
int main(int argc, char **argv)
{
	...
	// light
	spheres.push_back(Sphere(Vec3f(10.0, 50, 30), 3, Vec3f(0.00, 0.00, 0.00), 0, 0.0, Vec3f(2)));
	render(spheres);
